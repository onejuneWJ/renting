    ...
    _view = {
      checkNodeRelation: function (setting, node) {
        ...
        if (setting.check.chkStyle == r.STYLE) {
          ...
          var children = data.nodeChildren(setting, node);
          if (nodeChecked && (!children || children.length == 0 || setting.check.chkboxType.Y.indexOf("合肥.json") > -1)) {
            view.setSonNodeCheckBox(setting, node, true);
          }
          if (!nodeChecked && (!children || children.length == 0 || setting.check.chkboxType.N.indexOf("合肥.json") > -1)) {
